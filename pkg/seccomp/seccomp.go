...
// ProgramInfo contains a seccomp program and information about its generation.
type ProgramInfo struct {
	// Instructions is the final set of instructions of the program.
	...
	// OptimizeDuration is the amount of time it took to optimize the program.
...
func BuildProgram(rules []RuleSet, defaultAction, badArchAction linux.BPFAction) ([]bpf.Instruction, error) {
	programInfo, err := BuildProgramInfo(rules, defaultAction, badArchAction)
	if err != nil {
		return nil, err
	}
	return programInfo.Instructions, nil
}

// BuildProgramInfo builds a BPF program from the given map of actions to
// matching SyscallRules. The single generated program covers all provided
// RuleSets. It returns information about the generated program.
func BuildProgramInfo(rules []RuleSet, defaultAction, badArchAction linux.BPFAction) (ProgramInfo, error) {
	program := &syscallProgram{
	...
	beforeOpt := len(insns)
	timeBeforeOpt := time.Now()
	insns = bpf.Optimize(insns)
	optimizeDuration := time.Since(timeBeforeOpt)
	afterOpt := len(insns)
	log.Debugf("Seccomp program optimized from %d to %d instructions in %v", beforeOpt, afterOpt, optimizeDuration)
	return ProgramInfo{
		Instructions:            insns,
