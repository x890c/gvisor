...
func extractRepeatedMatchers(rule SyscallRule) (SyscallRule, bool) {
	...
	type extractData struct {
		...
		// otherMatchersSig represents the signature of other matches, with
...
// recursivelyOptimizeSyscallRule losslessly optimizes a SyscallRule and all
// its children using the given optimization functions.
// It returns the optimized syscall rule, along with a boolean indicating
// whether anything about the rule has changed.
func recursivelyOptimizeSyscallRule(rule SyscallRule, funcs []ruleOptimizerFunc) (SyscallRule, bool) {
	changed := false
	rule.Recurse(func(subRule SyscallRule) SyscallRule {
		newRule, subChanged := recursivelyOptimizeSyscallRule(subRule, funcs)
		if subChanged {
			changed = true
		}
		return newRule
...
func optimizeSyscallRuleFuncs(rule SyscallRule, funcs []ruleOptimizerFunc) SyscallRule {
	for changed := true; changed; {
		rule, changed = recursivelyOptimizeSyscallRule(rule, funcs)
	}
	return rule
}
