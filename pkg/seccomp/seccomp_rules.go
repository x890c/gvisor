...
// Render implements `HalfValueMatcher.Render`.
func (halfAnyValue) Render(program *syscallProgram, labelSet *labelSet) {
...
// Render implements `HalfValueMatcher.Render`.
func (heq halfEqualTo) Render(program *syscallProgram, labelSet *labelSet) {
...
// Render implements `HalfValueMatcher.Render`.
func (hns halfNotSet) Render(program *syscallProgram, labelSet *labelSet) {
...
// Render implements `HalfValueMatcher.Render`.
func (hmeq halfMaskedEqual) Render(program *syscallProgram, labelSet *labelSet) {
	program.Stmt(bpf.Alu|bpf.And|bpf.K, hmeq.mask)
	program.IfNot(bpf.Jmp|bpf.Jeq|bpf.K, hmeq.value, labelSet.Mismatched())
	program.JumpTo(labelSet.Matched())
}

// splittableValueMatcher is a ValueMatcher that can be split into two
// *independent* 32-bit value matchers.
// This enables some optimizations to occur.
type splittableValueMatcher interface {
	ValueMatcher

	// Split returns two `HalfValueMatcher`: the first one for the high bits,
	// the second one for the low bits.
	Split() (HalfValueMatcher, HalfValueMatcher)
}

// renderSplittable is a helper function for the `ValueMatcher.Render`
// implementation of `splittableValueMatcher`s.
// It assumes that *both* `HalfValueMatcher`s must match for the value
// to be matched.
func renderSplittable(svm splittableValueMatcher, program *syscallProgram, labelSet *labelSet, value matchedValue) {
	highMatcher, lowMatcher := svm.Split()

	// We render the "low" bits first on the assumption that most syscall
	// arguments fit within 32-bits, and those most rules actually only
	// care about the value of the low 32 bits. This way, we only check the
	...
	value.LoadLow32Bits()
	lowMatcher.Render(program, lowLabels)
	...
	highMatcher.Render(program, labelSet.Push("high", labelSet.Matched(), labelSet.Mismatched()))
...
func (av AnyValue) Render(program *syscallProgram, labelSet *labelSet, value matchedValue) {
	renderSplittable(av, program, labelSet, value)
}

// Split implements `splittableValueMatcher.Split`.
func (AnyValue) Split() (HalfValueMatcher, HalfValueMatcher) {
	return halfAnyValue{}, halfAnyValue{}
}
...
// Split implements `splittableValueMatcher.Split`.
func (eq EqualTo) Split() (HalfValueMatcher, HalfValueMatcher) {
	return halfEqualTo(high32Bits(uintptr(eq))), halfEqualTo(low32Bits(uintptr(eq)))
}
...
func (ne NotEqual) Render(program *syscallProgram, labelSet *labelSet, value matchedValue) {
	...
	renderSplittable(EqualTo(ne), program, labelSet.Push("inverted", labelSet.Mismatched(), labelSet.Matched()), value)
}
...
// Split implements `splittableValueMatcher.Split`.
func (NonNegativeFD) Split() (HalfValueMatcher, HalfValueMatcher) {
	...
	return halfEqualTo(0), halfNotSet(1 << 31)
}
...
// Split implements `splittableValueMatcher.Split`.
func (me maskedEqual) Split() (HalfValueMatcher, HalfValueMatcher) {
	highMask := high32Bits(me.mask)
	highMatcher := HalfValueMatcher(halfMaskedEqual{highMask, high32Bits(me.value)})
	if highMask == 0 {
		highMatcher = halfAnyValue{}
	} else if highMask == 0xffffffff {
		highMatcher = halfEqualTo(high32Bits(me.value))
	}
	lowMask := low32Bits(me.mask)
	lowMatcher := HalfValueMatcher(halfMaskedEqual{lowMask, low32Bits(me.value)})
	if lowMask == 0 {
		lowMatcher = halfAnyValue{}
	} else if lowMask == 0xffffffff {
		lowMatcher = halfEqualTo(low32Bits(me.value))
	}
	return highMatcher, lowMatcher
}
